...
// Make the left hand matrix and right matrix
template <typename Scalar>
...
struct TestSet final {
  ...
  void Run() {
    MakeZeroPoints(); // Get the zero points here.
    MakeLhsRhs();
    MakeSpec();
    MakeOtherParams();
    MakeResultPaths();
 ...
 public:
  ...
  int cols = 0;
  int depth = 0;
  Order lhs_order = Order::kRowMajor; // The lhs_order is rowMajor.
  Order rhs_order = Order::kColMajor; // The rhs_order is colMajor.
  Order dst_order = Order::kColMajor; // The dst_order is colMajor.
  ...
  DstScalar dst_zero_point = 0; // Set the destination zero point.

  std::vector<AccumScalar> per_channel_multiplier_fixedpoint;
  std::vector<int> per_channel_multiplier_exponent;

  StorageMatrix<LhsScalar> lhs; // lhs stores the left matrix.
  StorageMatrix<RhsScalar> rhs; // rhs stores the right matrix.
  Spec spec;
  std::vector<AccumScalar> bias_data; // The bias_data uses the AccumScalar data type.
...
void TestSet<LhsScalar, RhsScalar, SpecType>::MakeLhsRhs() {
  ...
  MakeRandom(rows, depth, lhs_order, lhs_zero_point, layout_style,
             RandomRange::kOffCenterAvoidMinValue, &lhs); // lhs -> StorageMatrix
  MakeRandom(depth, cols, rhs_order, rhs_zero_point, layout_style,
             RandomRange::kGeneral, &rhs); // rhs -> StorageMatrix
...
// So, here we can use environment variable.
